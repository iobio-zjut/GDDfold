...
ClassicAbinitio::read_distance(){
	...
	ifstream LT("distance_profile.txt");
    string line;
	Size line_num( 0 );
	Size res1( 0 );
	Size res2( 0 );
	multimap<Real, pair<pair<Size, Size>, vector<Real> > > splines_map;
...
//liujingw
Real 
ClassicAbinitio::min_best_generation(vector< Real >& population_Dscore)
{
	Real index_best( 0 );
	Real min = population_Dscore[0];
	for (Real i = 0; i < NP_; ++i)
	{
		if(min > population_Dscore[i])
		{
			min = population_Dscore[i];
			index_best = i;
		}
	}
	return index_best;
...
//liujingw
Real 
ClassicAbinitio::min_value_generation(vector< Real >& population_Dscore)
{
	Real min=population_Dscore[0];
	for (Size i = 0; i < NP_; ++i)
	{
...
ClassicAbinitio::k_mediods( const Size& k_number, vector<core::pose::Pose> &population, Size const &population_size, vector<Real> &state_tag){
	...
	while(clusterChanged && num<100)
	{
		...
		vector<Real> tag_cluster;
		for(Size med=0; med<k_number; med++)
		{
			Size minerror;
			for(Size i=0; i<population_size; i++)
			{
...
ClassicAbinitio::calculate_distance(core::pose::Pose &pose ){
	...
	for (Size i = 1; i <= pose.total_residue() - 8; ++i){
...
ClassicAbinitio::Output_last_generation(vector< core::pose::Pose >& population, vector< Real >& population_Dscore, char c){
	...
	population[min_RMSD_index].dump_pdb(name3+pdb);
}
...
void ClassicAbinitio::apply( pose::Pose & pose ) {
	...
	learn_period( population, NP_, proteinLength, G_LP_, *score_function_);

	vector<Real> statetag;
	vector<core::pose::Pose> Mediods;
	Mediods = k_mediods( Knumber, population, NP_, statetag);

	entropy_switch_strategy(population, NP_, proteinLength, G_global_, G_local_, *score_function_, statetag, Knumber, Mediods);
